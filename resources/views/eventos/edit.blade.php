...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Editar Evento: ' . $evento->nombre) }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto bg-white rounded-lg shadow p-8">
            ...
            <form action="{{ route('admin.eventos.update', $evento) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $evento->nombre) }}" required
                ...
                <div>
                    ...
                    <textarea name="descripcion" id="descripcion" rows="4"
                              class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500 @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $evento->descripcion) }}</textarea>
                    @error('descripcion')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="datetime-local" name="fecha_inicio" id="fecha_inicio" value="{{ old('fecha_inicio', $evento->fecha_inicio->format('Y-m-d\TH:i')) }}" required
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="fecha_fin" id="fecha_fin" value="{{ old('fecha_fin', $evento->fecha_fin->format('Y-m-d\TH:i')) }}" required
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="capacidad_participantes" id="capacidad_participantes" value="{{ old('capacidad_participantes', $evento->capacidad_participantes) }}" required min="1"
                    ...
                    <div>
                        ...
                        <input type="number" name="precio_inscripcion" id="precio_inscripcion" value="{{ old('precio_inscripcion', $evento->precio_inscripcion) }}" step="0.01" min="0"
                    ...
                    <div>
                        ...
                        <select name="id_deporte" id="id_deporte"
                            ...
                            @foreach($deportes as $deporte)
                                <option value="{{ $deporte->id_deporte }}" {{ old('id_deporte', $evento->id_deporte) == $deporte->id_deporte ? 'selected' : '' }}>
                                    {{ $deporte->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="id_cancha" id="id_cancha"
                            ...
                            @foreach($canchas as $cancha)
                                <option value="{{ $cancha->id_cancha }}" {{ old('id_cancha', $evento->id_cancha) == $cancha->id_cancha ? 'selected' : '' }}>
                                    {{ $cancha->nombre }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <input type="text" name="ubicacion" id="ubicacion" value="{{ old('ubicacion', $evento->ubicacion) }}"
                ...
                <div>
                    ...
                    <select name="estado" id="estado" required
                        ...
                        <option value="abierto" {{ old('estado', $evento->estado) == 'abierto' ? 'selected' : '' }}>Abierto</option>
                        <option value="cerrado" {{ old('estado', $evento->estado) == 'cerrado' ? 'selected' : '' }}>Cerrado</option>
                        <option value="en_progreso" {{ old('estado', $evento->estado) == 'en_progreso' ? 'selected' : '' }}>En Progreso</option>
                ...
                <div>
                    ...
                    @if($evento->foto)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $evento->foto) }}" alt="{{ $evento->nombre }}" class="h-32 w-32 object-cover rounded">
